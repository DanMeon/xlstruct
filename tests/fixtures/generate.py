...
PRODUCTS = [
    "Laptop Pro 15", "Wireless Mouse", "USB-C Hub", "Monitor 27in", "Keyboard MX",
    "Webcam HD", "Headset Pro", "Docking Station", "External SSD 1TB", "Graphics Tablet",
    "Desk Lamp LED", "Ergonomic Chair", "Standing Desk", "Cable Organizer", "Mousepad XL",
    "Portable Charger", "Bluetooth Speaker", "HDMI Cable 6ft", "Screen Protector", "Laptop Stand",
    "Surge Protector", "Whiteboard 48in", "Marker Set 12pk", "Desk Shelf Riser", "Privacy Screen",
]
...
def _save_with_formula_cache(
    ...
    with zipfile.ZipFile(buf, "r") as zin:
        ...
        name_to_file: dict[str, str] = {
            rid_to_name[rid]: rid_to_file[rid]
            for rid in rid_to_name
        ...
        out_buf = io.BytesIO()
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zin2, zipfile.ZipFile(out_buf, "w", zipfile.ZIP_DEFLATED) as zout:
...
def create_simple_employee_directory() -> list[dict]:
    ...
    headers = ["Employee ID", "Full Name", "Department", "Email", "Hire Date", "Annual Salary", "Active"]
...
def create_merged_financial_report() -> list[dict]:
    ...
    _save_with_formula_cache(wb, FIXTURES_DIR / "merged_financial_report.xlsx", {"Annual Report": formula_cache})
...
def create_large_transaction_log() -> list[dict]:
    ...
    headers = [
        "Txn ID", "Date", "Time", "Customer ID", "Customer Name",
        "Category", "Description", "Amount", "Payment Method", "Status",
    ]
    for col, h in enumerate(headers, 1):
...
def create_formula_budget_tracker() -> list[dict]:
    ...
    budgets = {
        "Engineering": 2400000, "Sales": 1800000, "Marketing": 1200000,
        "HR": 600000, "Finance": 500000, "Operations": 900000,
        "Legal": 400000, "Support": 700000, "Executive": 350000,
        "IT Infrastructure": 800000, "Product": 1100000, "Data Science": 950000,
    }
    ...
    for col in range(2, 18):
        ...
        if col == 17:
            ...
            ws.cell(row=total_row, column=col, value=f"=IF(B{total_row}>0,O{total_row}/B{total_row},0)")
    ...
    _save_with_formula_cache(wb, FIXTURES_DIR / "formula_budget_tracker.xlsm", {"Budget 2024": formula_cache})
...
def create_multi_level_header_sales() -> list[dict]:
    ...
    product_groups = {
        ...
        "Accessories": [
            "Portable Charger", "Bluetooth Speaker", "Laptop Stand",
            "Cable Organizer", "Mousepad XL", "Screen Protector",
        ],
        "Office Supplies": [
            "Whiteboard 48in", "Marker Set 12pk", "Surge Protector",
            "HDMI Cable 6ft", "Privacy Screen",
        ],
    }
    ...
    _auto_width(ws)
    _save_with_formula_cache(wb, FIXTURES_DIR / "multi_level_header_sales.xlsm", {"Regional Sales": formula_cache})
...
def create_complex_invoice_form() -> list[dict]:
    ...
    # * Tax
    ws.merge_cells(start_row=summary_start + 1, start_column=1, end_row=summary_start + 1, end_column=5)
    ws.cell(row=summary_start + 1, column=1, value="Tax")
    ws.cell(row=summary_start + 1, column=1).alignment = Alignment(horizontal="right")
    ws.cell(
        row=summary_start + 1, column=6,
    ...
    # * Grand Total
    ws.merge_cells(start_row=summary_start + 2, start_column=1, end_row=summary_start + 2, end_column=5)
    ...
    _save_with_formula_cache(wb, FIXTURES_DIR / "complex_invoice_form.xlsm", {"Invoice": formula_cache})
...
def create_legacy_inventory() -> list[dict]:
    ...
    headers = ["SKU", "Product Name", "Category", "Qty In Stock", "Unit Cost", "Reorder Level", "Last Restocked"]
...
def create_legacy_payroll() -> list[dict]:
    ...
    header_style = xlwt.easyxf("font: bold on; align: horiz center; pattern: pattern solid, fore_colour light_green;")
    money_fmt = xlwt.easyxf(num_format_str="#,##0.00")

    headers = [
        "Employee ID", "Name", "Department", "Pay Period", "Base Salary",
        "Overtime Hours", "Overtime Pay", "Bonus", "Deductions", "Net Pay",
    ]
    for col, h in enumerate(headers):
        ws.write(0, col, h, header_style)
...
def create_legacy_student_grades() -> list[dict]:
    ...
    header_style = xlwt.easyxf("font: bold on; align: horiz center; pattern: pattern solid, fore_colour light_yellow;")

    headers = [
        "Student ID", "Name", "Math", "English", "Science",
        "History", "Art", "Average", "Grade Letter", "Pass/Fail",
    ]
    for col, h in enumerate(headers):
        ws.write(0, col, h, header_style)
...
def create_edge_sparse_pivot() -> list[dict]:
    ...
    product_groups = {
        "Hardware": [
            "Laptop Pro 15", "Desktop Tower", "Monitor 27in", "Server Rack Unit",
        ],
        "Software": [
            "Office Suite License", "Antivirus Annual", "Cloud Storage Plan",
        ],
        "Networking": [
            "Router Enterprise", "Switch 48-port", "Access Point",
            "Firewall Appliance", "Cat6 Cable Box",
        ],
        "Services": [
            "Installation Service", "Annual Maintenance", "Consulting (hourly)",
        ],
    }
    ...
    month_names_ordered = ["Jan", "Feb", "Mar", "Apr", "May", "Jun",
                           "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
    ...
    for cat, products in product_groups.items():
        ...
        ws2.cell(row=cat_start_row, column=1).alignment = Alignment(
            vertical="center", horizontal="center", text_rotation=0,
        )

    _auto_width(ws2)
    ws2.column_dimensions["A"].width = 15
    ws2.column_dimensions["B"].width = 25
    _save_with_formula_cache(wb2, FIXTURES_DIR / "edge_sparse_pivot.xlsx", {"Sales Pivot": formula_cache})
...
def create_edge_mixed_layout() -> list[dict]:
    ...
    # * 25 order rows
    statuses = ["Shipped", "Shipped", "Shipped", "Processing", "Delivered", "Delivered", "Cancelled"]
    records: list[dict] = []
    formula_cache: dict[str, float | int | str] = {}
    grand_total_sum = 0.0
    for i in range(25):
    ...
    _save_with_formula_cache(wb, FIXTURES_DIR / "edge_mixed_layout.xlsx", {"Order Report": formula_cache})
...
def create_edge_nested_groups() -> list[dict]:
    ...
    headers = ["Department", "Team", "Sub-Team", "Employee ID", "Name", "Title", "Location", "Start Date"]
    for col, h in enumerate(headers, 1):
        ws.cell(row=3, column=col, value=h)
    _style_header_row(ws, 3, 1, len(headers))
    ...
    for dept_name, teams in org.items():
        ...
        for team_name, sub_teams in teams.items():
            ...
            for sub_name, members in sub_teams.items():
                ...
                if sub_end > sub_start:
                    ws.merge_cells(
                        start_row=sub_start, start_column=3,
                        end_row=sub_end, end_column=3,
                    )
            ...
            if team_end > team_start:
                ws.merge_cells(
                    start_row=team_start, start_column=2,
                    end_row=team_end, end_column=2,
                )
        ...
        if dept_end > dept_start:
            ws.merge_cells(
                start_row=dept_start, start_column=1,
                end_row=dept_end, end_column=1,
            )
